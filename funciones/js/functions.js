...
console.log( dniLetter( 345 ) === 'Invalid parameter');
...
array_lengths.push( findLongestWord( ["kitchen", "bedroom", "bathroom", "sittingroom", "leavingroom"]));
...
function charFreq( string ) {
	...
	for (i=0; i<abecedario.length; ++i){
		letter = abecedario[i];
		stringCutter = string.split(letter).join("");
		originalLength = string.length;
		actualLength = stringCutter.length;
		freq = originalLength - actualLength;
...
function beautifyLetters( array )
{
	...
	expreg1 = /\d+/;
	for (index in array){
		if (expregA.test(array[index]) == true) {
			array[index] = array[index].toUpperCase();
		} else if (expreg1.test(array[index]) == true){
...
function beautifyNumbers( array )
{
	...
	for (index in array){
		...
		while (array[index].length > 1){
			newArray = array[index].split("");
			count = 0;
			for (i=0; i<newArray.length; i++){
				newArray[i] = parseInt(newArray[i]);
				count += newArray[i];
			}
			array[index] = count.toString();
		}
		expreg = /\d+/;
		array[index] = (expreg.test(array[index]))? parseInt(array[index]): array[index];
	}
	return array;
}
...
function sortArray( array )
{
	...
	function compare(a,b){
		expreg = /\d+/;
		if (expreg.test(a) && !expreg.test(b)){
			return -1;
		}
		if (!expreg.test(a) && expreg.test(b)){
			return 1;
